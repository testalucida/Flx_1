		...
		_pSearch( NULL )
{
	...
	_pIn->textsize( labelsize() );
	_pIn->hide();
	_pIn->callback( Flx_Table::staticInputCallback, this );
	_pIn->when( FL_WHEN_ENTER_KEY_ALWAYS );
	end();
    
    _cellMenu.signalShortcut.
...
Fl_Input &Flx_Table::getEditor() {
	return *(new Fl_Input( 1, 1, 0, 0 ));
}

void Flx_Table::staticInputCallback( Fl_Widget *pW, void *pUserdata ) {
	((Flx_Table*)pUserdata)->inputCallback();
}

void Flx_Table::inputCallback() {
	endEdit();
}
...
void Flx_Table::drawColumnHeader( int col, const char* s, int x, int y, int w,
		int h ) {
...
void Flx_Table::draw_cell( TableContext context, int row, int col, int x, int y, int w, int h ) {
	...
	case CONTEXT_CELL: // Draw data in cells
		if( _pIn->visible() && row == _rowEdit &&  col == _colEdit ) return;
		{
			...
			if( _pData /*_model*/ ) {
				...
				fl_color (FL_BLACK);
				fl_draw( _pData->getValue( row, col ), x + 2, y, w, h, _vAlign[col], NULL, 0 );
...
void Flx_Table::makeColumnsFit() {
	for( int c = 0, cmax = cols(); c < cmax; c++ ) {
		...
		maxw += 6; //some extra space for the column header
...
/** called by static function event_callack */
void Flx_Table::event_callback() {
	_doCallback = false;
	setEventContext();
    
    if( Fl::event() == FL_PUSH ) take_focus();
...
void Flx_Table::showColumnHeaderContextMenu( int x, int y ) {
	_colhdrMenu.position( Fl::event_x(), Fl::event_y() );
	_colhdrMenu.setActive( MARK,  (_selectionMode == FLX_SELECTION_FREE) );
	_colhdrMenu.setActive( SORT_ASC, _isSortable );
	_colhdrMenu.setActive( SORT_DESC, _isSortable );
	createAllColumnsSubmenu();
	const Fl_Menu_Item* pItem = _colhdrMenu.popup();
	if( pItem ) {
...
void Flx_Table::createAllColumnsSubmenu() {
	char buf[256];
	...
	for( itr = headers.begin(); itr != headers.end(); itr++ ) {
		...
		strcat( buf, "/" );
		strcat( buf, pHeader );
		_colhdrMenu.add( buf, 0, NULL, (void*)pHeader, 0 );
	}
}
...
void Flx_Table::showCellContextMenu( int x, int y ) {
	_cellMenu.setEntriesCountItem( _pData->getRowCount() );
    _cellMenu.setActive( PASTE, true );
	_cellMenu.position( Fl::event_x(), Fl::event_y() );
	const Fl_Menu_Item* pItem = _cellMenu.popup();
	if( pItem ) {
        ...
        signalContextMenuItemSelected.send( _cellMenu, miaction );
	}
}
...
int Flx_Table::handle( int evt ) {
	...
	if( Fl::event_key() == FL_Escape ) return rc;
	
	if( evt == FL_DRAG || evt == FL_KEYDOWN ) {
		event_callback();
		return 1;
	}
	return rc;
...
bool Flx_Table::checkSelection() {
	...
	if( _selectionMode == FLX_SELECTION_SINGLECELL 	|| _selectionMode == FLX_SELECTION_SINGLEROW
	...
	if( _eventContext.tableContext == CONTEXT_CELL ) {
		if( _eventContext.isMouseClick() || _eventContext.isArrowKey() || _eventContext.isDragging ) {
