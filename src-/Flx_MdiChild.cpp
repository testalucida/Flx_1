	...
	, _titleSize( 11 )
{
	...
	begin();
        ...
        _pTitleGroup->labelcolor( FL_WHITE );
        _pTitleGroup->end();
        ...
        _pCloseBtn->tooltip( "Fenster schlie�en" );
...
void Flx_TitleBar::draw() {
	Fl_Group::draw();
//	fl_color( FL_WHITE );
//	fl_draw( _title.get(), _pImageBox->x() + _pImageBox->w() + 6, y() + 16 );
}
	...
	, _y( -1 )
{
	...
	begin();
		...
		_pClientArea = new Fl_Group( _pClientScroll->x(), _pClientScroll->y(), _pClientScroll->w(),
                                    _pClientScroll->h() );
		_pClientArea->end();
        _pClientScroll->end();
