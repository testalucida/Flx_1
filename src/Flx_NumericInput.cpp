...
: Flx_Group( X, Y, W, H, pLabel ) 
{
    ...
    _pInp->signalChanged.connect< Flx_NumericInput, &Flx_NumericInput::onChanged >( this );
    _pInp->signalEnterPressed.connect< Flx_NumericInput, &Flx_NumericInput::onEnter >( this );
    _pInp->signalTabPressed.connect< Flx_NumericInput, &Flx_NumericInput::onTab >( this );
    //_pInp->callback( Flx_NumericInput::staticCallback, this );
	//_pInp->when( FL_WHEN_ENTER_KEY_ALWAYS | FL_WHEN_CHANGED );
    _pInp->hide();
...
void Flx_NumericInput::onChanged( Flx_Input&, ActionParm &parm ) {
    signalChanged.send( *this, parm );
}
void Flx_NumericInput::onEnter( Flx_Input&, ActionParm &parm ) {
    signalEnterKey.send( *this, parm );
}
void Flx_NumericInput::onTab( Flx_Input&, ActionParm &parm ) {
    signalTabKey.send( *this, parm );
}
